...
global_step = 0
NUM_ACTIONS = 20 # For now we have dynamic action space. This means that we consider all nodes as actions but removing the repeated paths
K_path = True
...
class PPOActorCritic:
    ...
    def pred_action_distrib_sp(self, env, source, destination):
        ...
        if K_path == True:
            middlePointList = range(NUM_ACTIONS)
        else:
        ...
        while itMidd < len(middlePointList):
            if K_path == True:
                env.mark_action_sp(source, destination, source, destination, middlePointList[itMidd])
            else:
