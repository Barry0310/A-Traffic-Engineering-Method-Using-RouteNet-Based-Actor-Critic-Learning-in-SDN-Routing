...
if __name__ == "__main__":
    ...
    ppo = "Enero_3top_15_B_PATH_LINK_TEST_kp"
    method = "SAC_PL_KP"
    method2 = "PPO_PL_KP+LS"
    differentiation_str = args.d[0]
    folder_dir = args.f[0]
    num_topologies_x_link_failure = args.num_topologies[0]

    our_fail_2 = np.zeros(num_topologies_x_link_failure * TMs)
    our_fail_4 = np.zeros(num_topologies_x_link_failure * TMs)
    ...
    PPO_fail_2 = np.zeros(num_topologies_x_link_failure*TMs)
    PPO_fail_4 = np.zeros(num_topologies_x_link_failure*TMs)
    PPO_fail_6 = np.zeros(num_topologies_x_link_failure*TMs)
    PPO_fail_8 = np.zeros(num_topologies_x_link_failure*TMs)
    ...
    axis_counter = 0
    topology_Name = ""
    enero_directory = folder_dir+'/'+enero
        ...
        for subdir, dirs, files in os.walk(dir_to_topology_rewards):
            for file in files:
                if file.endswith((".pckl")):
                    ...
                    if num_links_failure==1:
                        PPO_fail_2[topology_id*TMs+tm_id] = results[3]
                    elif num_links_failure==2:
                        PPO_fail_4[topology_id*TMs+tm_id] = results[3]
                    elif num_links_failure==3:
                        PPO_fail_6[topology_id*TMs+tm_id] = results[3]
                    elif num_links_failure==4:
                        PPO_fail_8[topology_id*TMs+tm_id] = results[3]
    ...
    dd2_aux = np.zeros((TMs,5))
    dd4_aux = np.zeros((TMs,5))
    dd6_aux = np.zeros((TMs,5))
    dd8_aux = np.zeros((TMs,5))

    dd2 = pd.DataFrame(columns=[method,method+'+LS', method2, 'PPO_L_SP+LS','DEFO', 'Number Link Failures'])
    dd4 = pd.DataFrame(columns=[method,method+'+LS', method2, 'PPO_L_SP+LS','DEFO', 'Number Link Failures'])
    dd6 = pd.DataFrame(columns=[method,method+'+LS', method2, 'PPO_L_SP+LS','DEFO', 'Number Link Failures'])
    dd8 = pd.DataFrame(columns=[method,method+'+LS', method2, 'PPO_L_SP+LS','DEFO', 'Number Link Failures'])
        ...
        for topology_id in range(num_topologies_x_link_failure):
            ...
            for tm_id in range(TMs):
                if num_fail==2:
                    ...
                    dd2_aux[tm_id,3]=fail_2[topology_id*TMs+tm_id]
                    dd2_aux[tm_id,4]=defo_fail_2[topology_id*TMs+tm_id]
                    dd2_aux[tm_id,2]=PPO_fail_2[topology_id*TMs+tm_id]
                if num_fail == 4:
                    dd4_aux[tm_id, 0] = our_fail_4[topology_id * TMs + tm_id]
                    dd4_aux[tm_id, 1] = our_ls_fail_4[topology_id * TMs + tm_id]
                    dd4_aux[tm_id,3]=fail_4[topology_id*TMs+tm_id]
                    dd4_aux[tm_id,4]=defo_fail_4[topology_id*TMs+tm_id]
                    dd4_aux[tm_id,2]=PPO_fail_4[topology_id*TMs+tm_id]
                if num_fail == 6:
                    dd6_aux[tm_id, 0] = our_fail_6[topology_id * TMs + tm_id]
                    dd6_aux[tm_id, 1] = our_ls_fail_6[topology_id * TMs + tm_id]
                    dd6_aux[tm_id,3]=fail_6[topology_id*TMs+tm_id]
                    dd6_aux[tm_id,4]=defo_fail_6[topology_id*TMs+tm_id]
                    dd6_aux[tm_id,2]=PPO_fail_6[topology_id*TMs+tm_id]
                if num_fail == 8:
            ...
            if num_fail == 4:
                dd4.loc[it] = [np.mean(dd4_aux[:,0]), np.mean(dd4_aux[:,1]), np.mean(dd4_aux[:,2]), np.mean(dd4_aux[:,3]), np.mean(dd4_aux[:,4]), num_fail]
            if num_fail == 6:
                dd6.loc[it] = [np.mean(dd6_aux[:,0]), np.mean(dd6_aux[:,1]), np.mean(dd6_aux[:,2]), np.mean(dd6_aux[:,3]), np.mean(dd6_aux[:,4]), num_fail]
            if num_fail == 8:
